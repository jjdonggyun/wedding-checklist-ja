...
import type { BudgetItem, WeddingData } from "@/types/wedding";

type Props = {
  data: WeddingData;
  selectedPackage?: BudgetItem;
...
export function Dashboard({ data, selectedPackage, onOpenView }: Props) {
  ...
  const soon = data.template.checklist
    ...
    .filter((item) => item.deadline && new Date(item.deadline).getTime() - now <= 30 * dayMs)
    .slice(0, 6);
  const undecided = data.template.checklist.filter(
    (item) => item.undecided && !data.userState.checklist[item.id]?.completed,
  );
  const caution = data.template.checklist.filter(
    (item) => item.caution && !data.userState.checklist[item.id]?.completed,
  );
  const payments = data.template.budget
    .filter((item) => item.dueDate && !data.userState.budget[item.id]?.paid)
    .slice(0, 5);
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        />
        <article className="rounded-lg border border-rose-100 bg-white p-5 shadow-sm sm:col-span-2">
        ...
        <article className="rounded-lg border border-rose-100 bg-white p-5 shadow-sm sm:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <button className="text-sm font-semibold text-rose-600" onClick={() => onOpenView("budget")}>
              {t(language, "budget")}
            </button>
          </div>
          <p className="mt-3 text-2xl font-semibold">
            {selectedPackage ? `${money(selectedPackage.amount)}원` : "-"}
          </p>
          {selectedPackage && <p className="mt-1 text-sm text-stone-500">{text(selectedPackage.title, language)}</p>}
        </article>
      </div>
      ...
      <div className="grid gap-4">
        ...
        <SummaryList title={t(language, "cautionItems")} items={caution.slice(0, 5).map((item) => text(item.title, language))} onClick={() => onOpenView("checklist")} count={caution.length} danger />
      </div>
    </section>
  );
}
...
function DdayCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <article className="rounded-lg border border-rose-100 bg-white p-5 shadow-sm">
      ...
      <p className="mt-2 text-4xl font-semibold text-rose-600">{value}</p>
...
function SummaryList({
  ...
  return (
    <article className="rounded-lg border border-rose-100 bg-white p-4 shadow-sm">
      ...
      <ul className="mt-3 space-y-2">
        {items.length ? (
          items.map((item) => (
            <li key={item} className={`rounded-md px-3 py-2 text-sm ${danger ? "bg-red-50 text-red-700" : "bg-stone-50 text-stone-700"}`}>
              {item}
            </li>
          ))
        ) : (
